...
class WithDynamicSerializerMixin(
    ...
    def __init__(
        ...
        if data is not fields.empty:
            ...
            for field_name, field in iter(self.get_all_fields().items()):
                if (
    ...
    def _dynamic_init(self, only_fields, include_fields, exclude_fields):
        ...
        if exclude_fields == '*':
            ...
            include_fields = set(
                list(include_fields) + [
                    field for field, val in iter(self.request_fields.items())
                    if val or val == {}
                ]
            )
    ...
    def _all_fields(self):
        ...
        if (
            ...
            for k, field in iter(all_fields.items()):
    ...
    def _get_flagged_field_names(self, fields, attr, meta_attr=None):
        ...
        return {
            name for name, field in iter(fields.items())
    ...
    def get_fields(self):
        ...
        if request_fields:
            for name, include in iter(request_fields.items()):
    ...
    def _link_fields(self):
        ...
        if 'exclude_links' in query_params:
            ...
            all_fields = self.get_all_fields()
            return {
                name: field for name, field in iter(all_fields.items())
