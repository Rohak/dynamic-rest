...
def has_joins(queryset):
    ...
    """
    for join in iter(queryset.query.alias_map.values()):
        if join.join_type:
            return True
    return False
...
class DynamicFilterBackend(BaseFilterBackend):
    ...
    def _get_requested_filters(self, **kwargs):
        ...
        for spec, value in iter(filters_map.items()):
    ...
    def _filters_to_query(self, includes, excludes, serializer, q=None):
        ...
        def rewrite_filters(filters, serializer):
            out = {}
            for k, node in iter(filters.items()):
        ...
        if excludes:
            ...
            for k, v in iter(excludes.items()):
                q &= ~Q(**{k: v})
        return q
    ...
    def _build_implicit_prefetches(
        ...
        for source, remainder in iter(requirements.items()):
