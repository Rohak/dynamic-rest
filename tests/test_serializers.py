...
class TestDynamicSerializer(TestCase):
    ...
    def test_get_all_fields(self):
        s = GroupSerializer()
        all_keys1 = iter(s.get_all_fields().keys())
        f2 = s.fields
        all_keys2 = iter(s.get_all_fields().keys())
        expected = ['id', 'name']
        self.assertEqual(list(iter(f2.keys())), expected)
    ...
    def test_get_fields_with_only_fields(self):
        ...
        self.assertEqual(list(iter(serializer.fields.keys())), expected)
    ...
    def test_get_fields_with_include_fields(self):
        include = ['permissions']
        expected = set(
            iter(UserSerializer().get_fields().keys())
        ) | set(include)
        serializer = UserSerializer(
            include_fields=include
        )
        self.assertEqual(set(iter(serializer.fields.keys())), expected)
